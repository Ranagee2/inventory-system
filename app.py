...
TEXT = {
    "en": {
        "title": "JANJUA GROUP OF INDUSTRIES",
        "home": "Home",
        "products": "Products",
        ...
        "search_customer_placeholder": "Search by name, WhatsApp or Facebook",
        "search_customer_hint": "Use the search button to quickly find an already added customer.",
        "customer_not_found": "Customer not found.",
        "add_row": "Add Row",
        "remove_row": "Remove",
        "receipt_rows_hint": "Rows are added automatically as you fill them.",
    ...
    "ur": {
        ...
        "search_customer": "کسٹمر تلاش کریں",
        "search_customer_placeholder": "نام، واٹس ایپ یا فیس بک سے تلاش کریں",
        "search_customer_hint": "پہلے سے شامل کسٹمر کو جلدی ڈھونڈنے کے لیے سرچ بٹن استعمال کریں۔",
        "customer_not_found": "کسٹمر نہیں ملا۔",
        "add_row": "قطار شامل کریں",
        "remove_row": "حذف کریں",
        "receipt_rows_hint": "جیسے جیسے آپ قطاریں بھریں گے، نئی قطاریں خود بنتی رہیں گی۔",
        "toggle_sidebar": "سائیڈ بار کھولیں/بند کریں",
    },
}
    ...
    if request.method == "POST":
        ...
        for pid_raw, qty_raw in zip(product_ids, quantities):
            submitted_rows.append(
                {
                    "product_id": (pid_raw or "").strip(),
                    "quantity": (qty_raw or "").strip(),
                }
            )
        ...
        for row in submitted_rows:
            ...
            if pid not in selected_rows:
                selected_rows[pid] = {"product": product, "qty": 0}
            selected_rows[pid]["qty"] += qty

        items_to_save = list(selected_rows.values())
        ...
        if errors:
            ...
            return render_template(
                ...
                products=products,
                customers_list=customer_list,
                receipt_rows=submitted_rows or [{}],
                selected_customer_id=customer_id_raw,
            )
        ...
        try:
            ...
            receipt = Receipt(
                ...
                customer_whatsapp=customer.whatsapp if customer else "",
                customer_facebook=customer.facebook if customer else "",
                subtotal_amount=0.0,
            ...
            for idx, entry in enumerate(items_to_save):
                product = entry["product"]
                qty = entry["qty"]
